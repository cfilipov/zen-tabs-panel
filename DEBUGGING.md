...
README screenshots are generated by:

```bash
npm run screenshots:readme
```

The script launches a separate Zen profile on debugger port `6100`, installs a
fresh package from `dist/`, creates enough fixture workspaces to keep the
profile at 10 workspaces, opens deterministic fixture tabs, captures dark and
light variants, combines them with a diagonal split, and writes the README PNGs.
Pass `--theme light` or `--theme dark` to capture only one theme instead. The
default captures retina-scale pixels
(`--capture-scale 2`) and writes PNG density metadata so image viewers that
honor pHYs display the image at the menu's CSS-size point dimensions. Use
`--capture-scale 1` for true 1x files, or `--no-density` if the density metadata
is not wanted. Use `--output-dir /tmp/ergozen-shots` for a dry run,
`--reset-profile` to recreate the screenshot profile, `--workspace-count 10` to
change the fixture workspace count, and `--keep-open` when debugging the
capture profile.
