...
.zen-view-splitter-header-container:hover {
  opacity: 1 !important;
    ...
    function getBridgeBufferLength() {
      try { return chordSession ? chordSession.getBridgeBufferLength() : 0; } catch (e) { return 0; }
    }
    ...
    function currentViewName() {
      return overlayController ? overlayController.getCurrentView() : null;
    }

    function currentViewParams() {
      return overlayController ? overlayController.getCurrentParams() : {};
    }
    ...
    function trackChordOpenView(view) {
      chordSession.recordOpenView(view);
    }
    ...
    function forwardKeyToPopup(keyData) {
      ...
      if (!isPopupReady()) {
        if (hasBridgeBuffer()) {
          const buffered = pushBridgeKey(keyData);
          debugChordTrace("forward-key-buffered", { key: keyData && keyData.key, buffered });
          if (buffered === 1) armBufferedRevealTimer();
        }
        return;
      }
      const w = getWin();
    ...
    function recordModelRowIntentReplay(view, chordKey, switchToTarget, params) {
      if (!chordKey) return;
      chordSession.recordModelRowIntent(view, chordKey, switchToTarget, cloneReplayParams(params));
    }
